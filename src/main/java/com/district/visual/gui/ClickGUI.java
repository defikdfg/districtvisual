...
public class ClickGUI extends Screen {
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        ctx.drawText(textRenderer, "districtvisual v1.0.0", sx, sy, 0xFF7B61FF, true);
        sy += 20;
        ...
        for (int i = 0; i < categories.length; i++) {
            ...
            ctx.drawText(textRenderer, categories[i].name(), tx + 4, sy + 8, 0xFFFFFFFF, true);
        }
        sy += CAT_H + 4;
        ...
        for (int i = scroll; i < Math.min(mods.size(), scroll + 18); i++) {
            ...
            ctx.drawText(textRenderer, m.getName(), sx + 8, sy + 8, 0xFFFFFFFF, true);
            int statusColor = m.isEnabled() ? 0xFF44FF44 : 0xFFFF4444;
            ctx.drawText(textRenderer, m.isEnabled() ? "ON" : "OFF", sx + PANEL_W - 28, sy + 8, statusColor, true);
            sy += PANEL_H + 1;
        }

        ctx.drawText(textRenderer, "Click=toggle  Scroll=scroll  RShift=close", 15, this.height - 12, 0xFF555555, false);
